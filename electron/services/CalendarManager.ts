...
export class CalendarManager extends EventEmitter {
            ...
            const server = http.createServer(async (req, res) => {
                try {
                    if (req.url?.startsWith('/auth/callback')) {
                        ...
                        if (code) {
                            res.end(`Authentication successful! You can close this window and return to ${ELECTRON_BRAND.appName}.`);
                            server.close();
    ...
    private showNotification(event: CalendarEvent) {
        ...
        const notif = new Notification({
            ...
            body: `"${event.title}" starts in 2 minutes. Start ${ELECTRON_BRAND.shortAppName}?`,
            actions: [
