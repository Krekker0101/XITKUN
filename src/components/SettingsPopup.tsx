...
import { BRAND, hasConfiguredLink } from '../config/brand';
...
const SettingsPopup = () => {
    ...
    useEffect(() => {
        ...
        if (isFirstRender.current) {
            ...
            void window.electronAPI.setGroqFastTextMode(useGroqFastText).catch(console.error);
            return;
        }
            ...
            <div ref={contentRef} className={`w-[200px] max-h-[280px] backdrop-blur-md border rounded-[16px] overflow-hidden shadow-2xl p-2 flex flex-col animate-scale-in origin-top-left ${popupPanelClass}`}>
                ...
                <div className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group cursor-default ${itemHoverClass}`}>
                    <div className="flex items-center gap-3">
                        ...
                        />
                        <span className={`text-[12px] font-medium transition-colors ${isUndetectable ? (isLightTheme ? 'text-slate-950' : 'text-white') : labelInactiveClass}`}>{isUndetectable ? 'Protected' : 'Standard'}</span>
                    </div>
                    <button
                        onClick={() => {
                ...
                <div className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group interaction-base interaction-press ${itemHoverClass}`}>
                    ...
                    <div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        ...
                        {(shortcuts.toggleVisibility || ['Ctrl', 'B']).map((key, index) => (
                ...
                <div className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group interaction-base interaction-press ${itemHoverClass}`}>
                    ...
                    <div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        ...
                        {(shortcuts.takeScreenshot || ['Ctrl', 'H']).map((key, index) => (
                            <div key={index} className={`px-1.5 py-0.5 rounded border text-[10px] font-medium min-w-[20px] text-center ${shortcutKeyClass}`}>
                                {key}
                            </div>
                        ))}
                ...
                {hasConfiguredLink(BRAND.supportUrl) && (
                    <div
                        onClick={() => void window.electronAPI.openExternal(BRAND.supportUrl)}
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`text-[12px] transition-colors ${isLightTheme ? 'text-slate-700 group-hover:text-pink-700' : 'text-slate-400 group-hover:text-pink-100'}`}>Support {BRAND.ownerName}</span>
                        </div>
