...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.electronAPI.profileGetStatus().then(s => setHasProfile(s.hasProfile)).catch(() => {});
    ...
    const removeWarning = window.electronAPI.onIncompatibleProviderWarning((data) => {
      ...
      {isLauncherMainView && !isSettingsOpen && (
        <>
          ...
          <RemoteCampaignToaster
            ...
            campaign={typeof activeAd === 'object' && activeAd !== null ? activeAd : undefined}
            onDismiss={dismissAd}
          />
        </>
      )}
