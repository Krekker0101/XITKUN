...
const getLogFile = (): string | null => {
  ...
  try {
    _logFile = path.join(app.getPath('documents'), ELECTRON_BRAND.logFileName);
    return _logFile;
  } catch {
...
export class AppState {
  ...
  public showTray(): void {
    ...
    this.tray.setToolTip(ELECTRON_BRAND.appName)
    this.updateTrayMenu();
  ...
  public updateTrayMenu() {
    ...
    const contextMenu = Menu.buildFromTemplate([
      {
        label: `Show ${ELECTRON_BRAND.appName}`,
        click: () => {
          this.centerAndShowWindow()
        }
      },
  ...
  private _applyDisguise(mode: 'terminal' | 'settings' | 'activity' | 'none'): void {
    ...
    if (isWin) {
      ...
      app.setAppUserModelId(`${ELECTRON_BRAND.appUserModelIdBase}.${mode}`);
    }
