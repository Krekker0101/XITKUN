...
const Cropper: React.FC = () => {
    ...
    const commitSelection = useCallback((sp: { x: number, y: number } | null, cp: { x: number, y: number } | null) => {
        ...
        if (width > MIN_SELECTION_SIZE && height > MIN_SELECTION_SIZE) {
            window.electronAPI.cropperConfirmed({ x, y, width, height });
        } else {
            setStartPos(null);
            setCurrentPos(null);
        }
    ...
    useEffect(() => {
        ...
        const cleanupIpc = window.electronAPI.onResetCropper((data: { hudPosition: { x: number; y: number } }) => {
            setStartPos(null);
            setCurrentPos(null);
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                window.electronAPI.cropperCancelled();
                setStartPos(null);
                setCurrentPos(null);
            }
        };
